...
class Shell {
    ...
    parseAssignments = async (prefix) => {
        switch(prefix.type) {
            case "AssignmentWord":
                ...
                this.environment[environmentVar] = prefix.hasOwnProperty('expansion') ? expanded : split.join('');
                break;
            default:
                this.returnCode = 1;
    ...
    executeBash = async (bash) => {
        ...
        try {
            try {
                ast = parse(bash, { resolveEnv: this.resolveEnv, resolveParameter: this.resolveParameter, resolveAlias: this.resolveAlias });
            } catch(e) {
