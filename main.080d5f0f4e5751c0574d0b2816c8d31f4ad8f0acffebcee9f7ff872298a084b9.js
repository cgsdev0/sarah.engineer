...
function createSpanWithClasses(str, classes = []) {
  const span = document.createElement("span");
  span.innerText = str;
...
Map:		${import_ansi_styles.default.underline.open}https://${data.hostname}${import_ansi_styles.default.underline.close}
      ...
      grep = async (...args) => {
        ...
        return this.shell.stdin.split("\n").filter((line) => {
          ...
          while (match = re.exec(line)) {
            ...
            colorizedLine += import_ansi_styles.default.redBright.open;
            colorizedLine += match[0];
            colorizedLine += import_ansi_styles.default.redBright.close;
...
`;
      ...
      env = () => {
        return Object.entries(this.shell.env).map(([k, v]) => `${k}="${v}"`).join("\n");
      };
      unalias = (arg) => {
  ...
  "ns-hugo:/home/runner/work/sarah.engineer/sarah.engineer/assets/js/input.js"(exports, module) {
    ...
    module.exports = (shell) => async () => {
      ...
      document.querySelector(".buttons .red").addEventListener("click", () => {
        const close = () => {
          document.querySelector("#header-wrapper").classList.remove("slide-to-top");
      ...
      function update(target) {
        setTimeout(() => {
          ...
          fake.textContent = "";
          const caret = target.selectionStart === target.selectionEnd || target.selectionDirection === "backward" ? target.selectionStart + 1 : target.selectionEnd + 1;
...
var import_filesystem = __toESM(require_filesystem());
...
var Shell = class {
  constructor() {
    ...
    this.returnCode = 0;
    this.stdin = "";
    this.fs = new import_filesystem.default();
    this.cwd = "/";
    this.cwd_p = this.fs.getFsRoot();
    this.commands = new import_commands.default(this, this.fs);
  }
  parseAssignments = async (prefix) => {
    switch (prefix.type) {
      case "AssignmentWord":
        ...
        const envVar = split.shift();
        this.env[envVar] = prefix.hasOwnProperty("expansion") ? expanded : split.join("");
        break;
      default:
        this.returnCode = 1;
...
`);
  ...
  execLogicalExpression = async (expr) => {
    ...
    if (this.returnCode && expr.op === "or" || !this.returnCode && expr.op === "and") {
      this.returnCode = 0;
